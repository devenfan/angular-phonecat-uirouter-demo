            ...
            $stateProvider
                ...
                .state("home", {
                    ...
                    /*
                     ...
                     template: '<p class="lead">Welcome to the Phonecat(UI-Router) Demo</p>' +
                        '<p>Use the menu above to navigate. ' +
                        'Pay attention to the <code>$state</code> and <code>$stateParams</code> values below.</p>' +
                        '<p>Click these links—<a href="#/c?id=nexus-s">Nexus S</a> or ' +
                        '<a href="#/p/dell-streak-7">Dell Streak 7</a>—to see a url redirect in action.</p>'
                     */
                ...
                .state('phones', {
                    ...
                    // This abstract state will prepend '/phones' onto the urls of all its children.
                    url: '/phones',
                    ...
                    resolve: {
                        AllPhones: ['PhoneService',
                            function (PhoneService) {
                                return PhoneService.queryAll();
                            }]
                    },

                    // You can pair a controller to your template. There *must* be a template to pair with.

                    controller: ['$scope', '$state', 'AllPhones', 'UtilService',
                        function ($scope, $state, AllPhones, UtilService) {

                            // Add a 'phones' field in this abstract parent's scope, so that all
                            // child state views can access it in their scopes.
                            // Please note: scope inheritance is not due to nesting of states,
                            // but rather choosing to nest the templates of those states.
                            // It's normal scope inheritance.
                            $scope.phones = AllPhones;

                            $scope.goToRandom = function () {
                                var randId = UtilService.newRandomKey($scope.contacts, "id", $state.params.phoneId);

                                // $state.go() can be used as a high level convenience method
                                // for activating a state programmatically.
                                $state.go('phones.detail', { phoneId: randId });
                            };
                        }]
                })
                ...
                .state('phones.detail', {
                    ...
                    // For example: Its url will end up being '/contacts/{contactId:[0-9]{1,8}}'.
                    // When the url becomes something like '/contacts/42' then this state becomes active
                    // and the $stateParams object becomes { contactId: 42 }.
                    url: '/{phoneId:[a-z0-9A-Z_-]+}',
                    ...
                    views: {
                        ...
                        '': {
                            templateUrl: 'partials/phones.detail.html',

                            /*
                            controller: ['$scope', '$stateParams', 'UtilService',
                                function ($scope, $stateParams, UtilService) {
                                    $scope.phone = UtilService.findById($scope.phones, $stateParams.phoneId);
                                }]
                            */

                            controller: ['$scope', '$stateParams', 'PhoneService',
                                function ($scope, $stateParams, PhoneService) {
                        ...
                        'hint@': {
                            template: 'This is phone.detail populating the "hint" ui-view'
                        },
                        ...
                        'menuTip': {
                            ...
                            templateProvider: ['$stateParams',
                                function ($stateParams) {
                                    ...
                                    return '<hr><small class="muted">Phone ID: ' + $stateParams.phoneId + '</small>';
                                }]
                ...
                //////////////////////////////
                /*
                .state('phones.detail.item', {
                    // So following what we've learned, this state's full url will end up being
                    // '/phones/{phoneId}/item/:itemId'. We are using both types of parameters
                    ...
                    views: {
                        ...
                        '': {
                            templateUrl: 'partials/phones.detail.item.html',

                            controller: ['$scope', '$stateParams', '$state', 'UtilService',
                                function ($scope, $stateParams, $state, UtilService) {

                                    $scope.item = UtilService.findById($scope.phone.items, $stateParams.itemId);
